...
function EditDeleteServiceForm({ serviceName, initialData, onClose, onServiceUpdated, onServiceDeleted }) {
  ...
  const handleEditService = async (event) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/service-update/${serviceName}`, {
        method: 'PUT',
        headers: {
      ...
      toast.success('Данные о сервисе успешно обновлены!'); 
    } catch (error) {
  ...
  const handleDeleteService = async () => {
    if (window.confirm('Are you sure you want to delete this service? This action cannot be undone.')) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/service-delete/${serviceName}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
